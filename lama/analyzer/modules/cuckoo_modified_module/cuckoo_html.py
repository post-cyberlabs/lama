"""
Cuckoo Modified HTML class

This class parse all indicators of CuckooModified and create an HTML report.
"""
...
class CuckooModifiedHtml(object):
    ...
    def make_html(content):
        ...
        for item in content:
            ...
            if item.name == "malscore":
                vms[item.option]['malscore'] = item.content
            elif item.name == "malfamily":
                vms[item.option]['malfamily'] = item.content
        ...
        for _, vm in vms.items():
            ...
            # add score
            if "malscore" in vm:
                html += CuckooModifiedHtml._score_make_html(vm['malscore'])
            else:
                # TODO error maybe ???
                html += "###########################"
            if "malfamily" in vm:
                html += "Family : {}.".format(vm['malfamily'])
            else:
                # TODO error maybe ???
                html += "###########################"
            # add file category
            ...
            if 'all' in vm:
                ...
                html += CuckooModifiedHtml._tar_make_html(vm['all'])
                html += "</li>"

            if 'pcap' in vm:
                html += "<h4>-> Pcap</h4>"
                html += "<li class=\"cuckoo_part_li\">"
                html += CuckooModifiedHtml._pcap_make_html(vm['pcap'])
                html += "</li>"
            ...
            if 'screenshots' in vm:
                ...
                html += "<li class=\"cuckoo_part_li\">"
                html += CuckooModifiedHtml._screenshot_make_html(vm['screenshots'])
                html += "</li>"
            ...
            if "signature" in vm:
                ...
                for item in reversed(vm['signature']):
                    html += CuckooModifiedHtml._signatures_make_html(item.content)
                html += "</li>"
            ...
            if "network" in vm:
                ...
                for item in vm['network']:
                    html += CuckooModifiedHtml._network_make_html(item.content)
                html += "</li>"
            ...
            if "process" in vm:
                ...
                for item in vm['process']:
                    html += CuckooModifiedHtml._process_make_html(item.content)
                html += "</li>"
            ...
            if "buff_yara" in vm:
                ...
                for item in vm['buff_yara']:
                    html += CuckooModifiedHtml._buff_yara_make_html(item.content)
                html += "</li>"
        ...
        for proto in decode_content:
            if not (proto == "pcap_sha256" or proto == "sorted_pcap_sha256")  and len(decode_content[proto])>=1:
                ...
                for item in decode_content[proto]:
                    # html += "<li><del>{}</del></li>".format(item)
                    if proto == "icmp":
                        ...
                        html += "<li>{} : {} -> {}</li>".format(escape(item['ip']), escape(item['hostname']), escape(item['country_name']))
                    elif proto == "http":
                        html += (
                            "<li>"
                            "<b>{}</b><br/>"
                        ...
                        html += "<li>ERROR PARSE CuckooModifiedHtml : {}</li>".format(proto)
                html += "</ul>"
                html += "</div>"
        html += "</div>"
        return html
    ...
    def _process_make_html(content):
        ...
        for proctree in decode_content:
            html += CuckooModifiedHtml._process_make_html_rec(proctree)
        return html

    def _process_make_html_rec(proctree):
        html = "<div class=\"processtree\">"
        html += "{} : {}".format(proctree['name'], proctree['environ']['CommandLine'])
        for child in proctree['children']:
            html += CuckooModifiedHtml._process_make_html_rec(child)
        html += "</div>"
        return html
